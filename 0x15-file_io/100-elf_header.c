 ...
 */
int _strncmpr(const char *s1, const char *s2, size_t n)
{
	for ( ; n && *s1 && *s2; --n, ++s1, ++s2)
	{
		if (*s1 != *s2)
...
/**
 * _clse - closes a file descriptor then prints error message upon failure
 ...
 * @fd: the file descriptor to refrom
 ...
 * @buffer: contains the entry point address
